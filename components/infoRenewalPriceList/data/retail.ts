export const getRenewalPriceTable = (services: string[], rootTable: string[][]) => {
  let filteredTable = [];
  const listIndex = services
    .map((item) => {
      const index = rootTable[0].indexOf(item);
...
export const getRenewalPriceTable2 = (services: services[], rootTable: string[][]) => {
